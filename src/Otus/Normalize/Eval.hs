...
-- object
evaluateTeleClosure :: Value -> TeleClosure -> EvalResult VTelescope
evaluateTeleClosure p cls = evaluateTelescope (clsTm cls) (clsEnv cls ||> p)

evaluateTeleClosureFresh :: TeleClosure -> EvalResult VTelescope
evaluateTeleClosureFresh cls = evaluateTelescope (clsTm cls) (freshVar' $ clsEnv cls)

evaluateClosure :: Value -> ObjClosure -> EvalResult Value
evaluateClosure p cls = evaluate (clsTm cls) (clsEnv cls ||> p)

evaluateClosureFresh :: ObjClosure -> EvalResult Value
evaluateClosureFresh cls = evaluate (clsTm cls) (freshVar' $ clsEnv cls)

evaluateTelescope :: Telescope -> Environment -> EvalResult VTelescope
evaluateTelescope tele env = case tele of
  Empty -> return VTNil
  ty :<| rest -> do
    vTy <- evaluate ty env
    return $ VTCons vTy (Closure env rest)
...
evaluate tm env = case tm of
  ...
  Record tele -> VRecord <$> evaluateTelescope tele env
...
evaluateMetaComputation ctm env = case ctm of
  MTyAnnotation t _ -> evaluateMetaComputation t env
  MPi dom eff cod -> do
